...
function KeyPad() {
  this.pad = [[], [1, 2, 3], [4, 5, 6], [7, 8, 9], [, 0], []]
    .map(arr => arr.map(num => String(num)));
}

KeyPad.prototype.maybe = function maybe(num) {
  const input = String(num);
  let row;
  for (let i = 1; i < 5; i += 1) {
    if (this.pad[i].includes(input)) row = i;
  }
  if (typeof row !== 'number') {
    return [];
  }
  const column = this.pad[row].indexOf(input);

  const checks = [
    this.pad[row - 1][column],
    this.pad[row][column - 1],
    String(num),
    this.pad[row][column + 1],
    this.pad[row + 1][column],
  ];
...
function getPINs(observed) {
  ...
  let output = [''];
  observed.split('').forEach((key) => {
    const combos = [];
    output.forEach((str) => {
      combos.push(...pad.maybe(key).map(maybe => str + maybe));
    });
    output = combos;
  });
  return output;
